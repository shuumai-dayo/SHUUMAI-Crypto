...
# TkinterDnD.Tk を継承してD&D機能を有効にし、ctk.CTk の機能も利用する
class App(TkinterDnD.Tk, ctk.CTk):
    def __init__(self):
        ...
        # --- Windowsのタスクバーにアイコンを正しく表示するための設定 ---
        if sys.platform == 'win32':
            myappid = 'mycompany.product.smaicrypt.1' # アプリケーション固有のID
        ...
        # --- アクションボタン --- #
        button_frame = ctk.CTkFrame(main_frame, fg_color="transparent") # fg_colorをtransparentに
    ...
    def handle_drop(self, event):
        ...
        # event.data はファイルパスのリストを含む文字列 "{C:/path/to/file1} {C:/path/to/file2}"
        # 最初の一つのファイルのみを処理する
        # 波括弧 {} を取り除き、最初のファイルパスを取得
        file_path = event.data.strip('{}')
        
        # ファイルが存在し、単一のファイルであることを確認
    ...
    def _run_encryption(self, file_path, password):
        ...
        if success:
            ...
            color = "#81C784" # 緑色
            # 元ファイルを削除するオプションが有効かチェック
            if self.delete_original_var.get():
                try:
                    os.remove(file_path)
                    message += " (元ファイル削除済み)"
                    # ファイルを削除したら、選択状態をリセット
                    self.selected_file_path = ""
